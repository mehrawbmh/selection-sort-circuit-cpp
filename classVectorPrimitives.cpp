...
void Mux3::evl() {
    if (i4->range(0,1) == "00") *o1 = *i1;
    else if (i4->range(0,1).ival() == 1) *o1 = *i2;
    else if (i4->range(0,1).ival() == 2) *o1 = *i3;
    else o1->fill('Z');
}
...
void upCounter::evl() {
    if (c->at(0) == "P") {
        if (l->at(0) == "1") { *internalCount = *d; cout << "loading to counter...\n";}
        else {
...
//		cout << "Memory read: @" << address.ival() << ": " << *Dout << "\n";
        ...
        if (write->at(0) == "1") {
			...
			cout << "Memory write: @" << address.ival() << ": " << *Din << "\n";
        }
    }
}
